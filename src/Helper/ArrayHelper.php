...
class ArrayHelper
{
    ...
    public static function getValue($array, $key, $default = null)
    {
        ...
        if (is_object($array)) {
            ...
            return $array->{'get'.StringHelper::asCamelCase($key)}();
     ...
     */
    public static function relationIndex(&$data, string $indexName, string $indexTarget)
    {
        if (!isset($data[$indexName]) || !is_array($data[$indexName])) {
            return;
        }

        $result = [];
//        $newIndexForItem = 0;
        foreach ($data[$indexName] ?? [] as $key => $contactAddress) {
            $newIndexAlias = $contactAddress[$indexTarget] ?? 'new_'.$key;
            $result[$newIndexAlias] = $contactAddress;
        }
    ...
    /**
     * Создание сущности по массиву атрибутов.
     *
     ...
     * @return object
     */
    private static function newFromArray(array $attributes, $entity)
    {
        foreach ($attributes as $name => $value) {
            $methodName = self::getSetterName($name, $entity);
            if (false !== $methodName) {
                $entity->{$methodName}($value); // Set or Add
            }
        }
     ...
     * @return false|string
     */
    private static function getSetterName($propertyName, $entity)
    {
        $prefixes = ['add', 'set'];
    ...
    public static function entityFromArray(string $entityName, array $attributes)
    {
        if (is_string($entityName) && class_exists($entityName)) {
            $entity = new $entityName();
        } else {
            throw new \Exception("Class '$entityName' not exists");
        }

        return self::newFromArray($attributes, $entity);
    }
}
...
interface Arrayable
{
     ...
     *                         in [[fields()]] will be ignored. If this parameter is empty, all fields as specified in
